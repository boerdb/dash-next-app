...
import { useTahomaLog } from "@/lib/hooks/use-tahoma";
import { METRIC_BY_KEY, operatorLabel } from "@/lib/tahoma/metrics";
import { ACTIONS } from "@/components/tahoma/TahomaLogCard.helpers";

export function TahomaLogCard() {
  const { entries, mutate } = useTahomaLog();

  const clear = async () => {
    await fetch("/api/tahoma/log", { method: "DELETE" });
    await mutate();
  };
        ...
        {entries.length === 0 ? (
          ...
          <ul className="divide-y divide-border-subtle">
            {entries.map((e) => {
              ...
              const actionLabel = ACTIONS.find((a) => a.value === e.action)?.label ?? e.action;
              return (
                <li key={e.id} className="py-2.5">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-sm text-foreground">
                        <span className="font-medium">{e.deviceName}</span>{" "}
