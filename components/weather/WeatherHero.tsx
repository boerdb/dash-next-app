...
export function WeatherHero({
  ...
  return (
    ...
    >
      <div className="relative z-10 text-center [text-shadow:0_1px_10px_rgba(0,0,0,0.65)]">
        <div className="mb-3 flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-xs text-white/85">
          ...
          {showWeatherSub ? (
            <>
              ...
              <span className="text-label font-medium uppercase text-white/85">{weatherLabel}</span>
            </>
          ) : null}
        </div>
        ...
        <h1 className="text-6xl font-bold tabular-nums leading-none text-white drop-shadow-[0_2px_16px_rgba(0,0,0,0.9)]">
          {temp}
          <sup className="ml-1 text-2xl font-normal text-white/85">°C</sup>
        </h1>

        <div className="mx-auto mt-4 flex max-w-md flex-wrap items-center justify-center gap-x-4 gap-y-1 rounded-2xl border border-white/30 bg-gradient-to-br from-sky-500/30 to-indigo-900/50 px-4 py-2.5 text-sm text-white shadow-[0_4px_20px_rgba(0,0,0,0.25)] ring-1 ring-white/10 backdrop-blur-md">
          {showWindChill ? (
            <span>
              Gevoel{" "}
          ...
          <span>
            {data.temp_min_time ? (
              <span className="text-caption mr-1 tabular-nums text-white/70">
                {data.temp_min_time}
              </span>
            ) : null}
            Min{" "}
            <strong className="font-semibold tabular-nums text-cyan-200">
              {data.temp_min_c}°
            </strong>
          </span>
          <span className="hidden text-white/40 sm:inline">|</span>
          <span>
            ...
            {data.temp_max_time ? (
              <span className="text-caption ml-1 tabular-nums text-white/70">
                {data.temp_max_time}
              </span>
            ) : null}
          </span>
